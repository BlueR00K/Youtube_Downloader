...
export default function App() {
    ...
    const download = async (targetUrl, format_id) => {
        try {
            setDownloading(true)
            setProgress(0)
    ...
    const downloadAll = async () => {
        try {
            ...
            const res = await axios.post(`${BACKEND}/api/downloads`, { urls }, {
                responseType: 'blob',
                onDownloadProgress: (evt) => {
    ...
    return (
        <div className="container py-4">
            ...
            <div className="mb-3">
                <label className="form-label">YouTube / URL(s) — one per line for multiple</label>
                <textarea className="form-control" rows={3} value={multiUrls} onChange={(e) => setMultiUrls(e.target.value)} placeholder="https://www.youtube.com/watch?v=..." />
            </div>

            <div className="mb-3">
                <button className="btn btn-primary me-2" onClick={fetchInfo} disabled={loading || (!url && !multiUrls)}>Get Info</button>
                <button className="btn btn-secondary" onClick={downloadAll} disabled={downloading || (!url && !multiUrls)}>Download All (ZIP)</button>
            </div>
                    ...
                    {info.batch ? (
                                ...
                                <div key={idx} className="card mb-3">
                                    <div className="card-body">
                                        <div className="d-flex gap-3 align-items-start mb-2">
                                            {it.thumbnails && it.thumbnails.length > 0 && (
                                                <img src={it.thumbnails[it.thumbnails.length - 1].url} alt="thumb" style={{ maxWidth: 160 }} />
                                            )}
                                            <div>
                                                        ...
                                                        <tbody>
                                                            {it.formats.map((f) => (
                                                                <tr key={f.format_id}>
                                                                    ...
                                                                    <td>
                                                                        <button className="btn btn-sm btn-success" onClick={() => download(it.url, f.format_id)} disabled={downloading}>{downloading ? 'Downloading...' : 'Download'}</button>
                                                                    </td>
                                                                </tr>
                                                            ))}
                                                        </tbody>
                        ...
                        <div>
                            <div className="d-flex gap-3 align-items-start mb-3">
                                {info.item.thumbnails && info.item.thumbnails.length > 0 && (
                                    <img src={info.item.thumbnails[info.item.thumbnails.length - 1].url} alt="thumbnail" style={{ maxWidth: 220, borderRadius: 8 }} />
                                )}
                                <div>
                                    ...
                                    <tbody>
                                        {info.item.formats.map((f) => (
                                            <tr key={f.format_id}>
                                                ...
                                                <td>{f.vcodec}</td>
                                                <td>
                                                    <button className="btn btn-sm btn-success" onClick={() => download(info.url || url, f.format_id)} disabled={downloading}>
